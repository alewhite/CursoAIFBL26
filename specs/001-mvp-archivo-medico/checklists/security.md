...
- [x] CHK001 - ¿Está enumerada de forma cerrada la lista de rutas que pueden atenderse sin sesión, de modo
      que la excepción a la autenticación por omisión sea auditable? [Hueco, Spec §FR-001]
- [x] CHK002 - ¿Está especificado el requisito de protección contra falsificación de solicitudes entre
      sitios para los formularios que mutan datos, más allá del atributo SameSite de la cookie?
      [Hueco, Spec §FR-008]
- [x] CHK003 - ¿Está definido si el cierre de sesión invalida únicamente la sesión en curso o todas las
      sesiones activas de la cuenta? [Hueco, Spec §FR-004]
- [x] CHK004 - ¿Está especificado que el identificador de sesión debe renovarse al autenticarse, para
      cerrar la fijación de sesión? [Hueco, Spec §FR-002]
- [x] CHK005 - ¿Están definidos requisitos mínimos de fortaleza para las contraseñas que el procedimiento
      administrativo asigna? [Hueco, Spec §FR-009, §FR-010]
- [x] CHK006 - ¿Está especificado el comportamiento esperado cuando una credencial almacenada quedó con
      parámetros de derivación por debajo del mínimo exigido? [Hueco, Spec §FR-009]
- [x] CHK007 - ¿Está definido el tratamiento del área de tránsito donde el archivo existe sin cifrar
      durante la validación: su ubicación, su protección y su limpieza? [Hueco, Spec §FR-030, §FR-035]
- [x] CHK008 - ¿Está especificado qué protección tienen los metadatos médicos en reposo, dado que el
      requisito de cifrado alcanza solo a los archivos? [Hueco, Spec §FR-035]
- [x] CHK009 - ¿Está definido un requisito de rotación o reemplazo de la clave de cifrado ante sospecha de
      compromiso? [Hueco, Spec §FR-036, §FR-068]
- [x] CHK010 - ¿Está especificado el requisito de forzar el uso de HTTPS en visitas posteriores, y no solo
      la redirección de la solicitud sin cifrar? [Hueco, Spec §FR-060]
- [x] CHK011 - ¿Está definido qué sí puede registrarse en logs —identificadores técnicos, códigos de
...
- [x] CHK013 - ¿Está especificada la política positiva de la caché del navegador —qué puede guardarse—, y
...
- [x] CHK020 - ¿Está especificado si un intento realizado durante el bloqueo extiende la ventana de 15
      minutos o la deja correr? [Ambigüedad, Spec §FR-007]
- [x] CHK021 - ¿Está definido qué identificador usa el inicio de sesión —nombre de usuario o correo—, dado
      que la elección cambia qué se puede enumerar desde afuera? [Ambigüedad, Spec §FR-002]
- [x] CHK022 - ¿Está definido qué significa "cuenta activa" a efectos del límite de cinco, y si una cuenta
...
- [x] CHK029 - ¿Se aplica el mismo criterio de aislamiento por propietario a todos los canales que
...
- [x] CHK030 - ¿Es consistente el requisito de no ejecutar contenido activo al visualizar con el de
...
- [x] CHK035 - ¿Puede verificarse el criterio de que ninguna ruta acepta una creación de cuenta sin
...
- [x] CHK040 - ¿Está especificado qué debe ocurrir si el borrado del contenido cifrado falla después de
